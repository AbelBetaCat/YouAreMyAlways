...
import static com.stone.app.dataBase.DataBaseError.ErrorType.PhoneNumberConflict;
...
//            memberData = dataBaseManager.AddMember(nickname, intent.getStringExtra("password"),  "", name,  gendrType, "11" +String.valueOf(phone), "");
            memberData = dataBaseManager.AddMember(nickname, intent.getStringExtra("password"),  "", name,  gendrType, intent.getStringExtra("phone"), "");

//                    "", name,  gendrType, "11" +intent.getStringExtra("phone"), "");
...
//            Log.i("TAG", "memberDatagetName " + memberData.getName()+"memberDatagetName " + memberData.getGender() );
            ...
            if(PhoneNumberConflict==dataBaseError.getErrorType()){

                ToastUtil.showToast(setInfomationActivity.this," 注册失败，账号已存在");
            }
            else {
                ToastUtil.showToast(setInfomationActivity.this," 注册失败，请检查你输入的手机号");
            }
